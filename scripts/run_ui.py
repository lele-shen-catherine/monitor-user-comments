...
HOME=HOME.replace("默认最多采集 100 个视频、每个视频 100 条一级评论。首次运行可能需要在浏览器中完成抖音登录或验证；程序不会读取或展示 Cookie。","默认执行快速监测（20 条高赞帖子 × 每帖最多 50 条评论）。结果页可选择深度监测。请在自己的浏览器中登录自己的抖音账号并完成平台要求的验证。")
...
def execute(job_id: str, cfg, keyword: str, mode: str):
    ...
    try:
        ...
        job.update(status="collecting",message=f"正在执行{mode_name}：{max_posts} 条高赞帖子，每帖最多 {max_comments} 条评论；请确认自己的浏览器已登录自己的抖音账号。")
