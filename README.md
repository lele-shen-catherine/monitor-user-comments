...
首次启动会自动配置 Python 3.11、项目虚拟环境、Node.js 和 MediaCrawler。SnowNLP 0.12.3 的离线安装包已经包含在 `vendor/`。首次安装 MediaCrawler 时需要阅读并接受其非商业学习许可证，下载量较大；以后启动会复用已经安装好的程序环境。

电脑需要已安装 Chrome 或 Edge。每位使用者必须在自己的浏览器中登录自己的抖音账号；仓库和安装包不包含任何人的登录信息。
...
- 每位使用者使用自己的抖音账号；仓库、安装包和报告均不包含登录 Cookie 或 Token。
